...
- **Repository policy aligned** with the Constitution — `README.md` and
  `XXXX-sgp-template.md` in this repository should be reconciled with the
  Constitution's policy choices by the maintainers, so that contributor-facing
  documentation and this ratified document do not diverge.

The `svmgov` and NCN dependencies are met at the time this SGP is filed;
repository-documentation alignment is expected to be reconciled by the
maintainers.
